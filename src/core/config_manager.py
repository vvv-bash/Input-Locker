...
class ConfigManager:
    ...
    def get_language(self) -> str:
        """Return configured UI language code (e.g. 'en' or 'es')."""
        lang = str(self.get('ui', 'language', 'en') or 'en').lower()
        return 'es' if lang.startswith('es') else 'en'

    def set_language(self, language: str) -> bool:
        """Persist UI language selection.

        Currently supports 'en' and 'es'; any other value falls back
        to English but is still stored so future locales can be added
    ...
    def get_unlock_pattern_id(self) -> str:
        """Return identifier of the emergency unlock pattern.

        The value is a small string such as 'arrows' or 'wasd'. The
        actual mapping to key codes is handled by the selective
        keyboard blocker.
        """
        return self.get('general', 'unlock_pattern', 'arrows') or 'arrows'

    def set_unlock_pattern_id(self, pattern_id: str) -> bool:
        """Persist the identifier of the emergency unlock pattern."""
        try:
            self.set('general', 'unlock_pattern', pattern_id)
            return self.save()
        except Exception as e:
            logger.error(f"Error setting unlock pattern: {e}")
            return False
