...
from ..utils.privileges import PrivilegeManager
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.btn_nav_diagnostics.clicked.connect(self._show_diagnostics)
        ...
        # Page 5: Diagnostics – now focused on devices, permissions and logs
        ...
        from PyQt6.QtWidgets import QTabWidget, QTextEdit, QPushButton
        self.diagnostics_tabs = QTabWidget()

        # Summary tab
        self.diagnostics_summary = QTextEdit()
        self.diagnostics_summary.setReadOnly(True)
        self.diagnostics_tabs.addTab(self.diagnostics_summary, "Summary")

        # Dependencies tab (existing widget)
        self.dependency_viewer = DependencyViewer()
        self.diagnostics_tabs.addTab(self.dependency_viewer, "Dependencies")

        # Performance tab (existing widget)
        self.profiling_viewer = ProfilingViewer()
        self.diagnostics_tabs.addTab(self.profiling_viewer, "Performance")
        ...
        self.btn_copy_report.setObjectName("btnSecondary")
        self.btn_copy_report.clicked.connect(self._copy_diagnostics_report)
        diag_btn_row.addWidget(self.btn_copy_report)
        diagnostics_layout.addLayout(diag_btn_row)
    ...
    def _show_diagnostics(self):
        """Switch to diagnostics page and refresh the summary text."""
        try:
            self._refresh_diagnostics_summary()
        except Exception:
            logger.exception("Failed to refresh diagnostics summary")
        self._switch_to_page(5)
        self.btn_nav_diagnostics.setChecked(True)

    def _refresh_diagnostics_summary(self):
        """Build a human-readable diagnostics summary for the UI.

        It combines device information, privilege checks and a small
        environmental snapshot so that users can quickly copy it when
        reporting bugs.
        """
        try:
            ...
            app_name = app.applicationName() if app is not None else "Input Locker"

            lines = []
            lines.append(f"{app_name} – Diagnostics summary")
            ...
            # Privileges
            info = PrivilegeManager.get_user_info()
            lines.append("Privileges:")
            lines.append(f"  User: {info.get('username')} (uid={info.get('uid')}, gid={info.get('gid')})")
            lines.append(f"  Effective: uid={info.get('euid')}, gid={info.get('egid')}")
            lines.append(f"  Is root: {info.get('is_root')}")
            lines.append("")
            ...
            lines.append(f"  Locked: {self.input_blocker.get_lock_status()}")
            lines.append(f"  Locked devices: {len(self.input_blocker.get_blocked_devices())}")
            lines.append("")

            # Config snapshot (hotkey + theme only; rest in config file)
            ...
            text = "\n".join(lines)
            if hasattr(self, "diagnostics_summary") and self.diagnostics_summary:
                self.diagnostics_summary.setPlainText(text)
        except Exception:
            logger.exception("Error building diagnostics summary")

    def _copy_diagnostics_report(self):
        """Copy the diagnostics summary to clipboard for bug reports."""
        try:
            from PyQt6.QtWidgets import QApplication

            if not hasattr(self, "diagnostics_summary") or not self.diagnostics_summary:
                return

            text = self.diagnostics_summary.toPlainText().strip()
            if not text:
                return
            ...
            QMessageBox.information(
                self,
                "Diagnostics copied",
                "A diagnostics report has been copied to the clipboard.\n"
                "You can paste it into an issue or support request.",
            )
        except Exception:
            logger.exception("Failed to copy diagnostics report")
